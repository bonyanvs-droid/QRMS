...
export function estimateAyahLineWeight(ayah: {
  ...
  let text = ayah.cleanText || ayah.text || '';
  // In Madani Mushaf, Basmalah is printed as a header line, not inside Ayah 1's verse text
  // (except in Al-Fatihah where Basmalah is verse 1).
  if (surahNumber > 1 && ayahNumber === 1) {
    text = text.replace(/^(بِسْمِ\s+ٱللَّهِ\s+ٱلرَّحْمَٰنِ\s+ٱلرَّحِيمِ|بسم\s+الله\s+الرحمن\s+الرحيم)\s*/, '');
  }
